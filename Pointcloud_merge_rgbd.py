...
class pointcloud_merging:
    def __init__(self):
        ...
        self.filename = 'ply/CapturePose_Tool0_Base0.csv'
        ...
        # self.tool2c_rt = np.array([[-0.010502, 0.999823, 0.012148, -0.0780857*1],
        #                            [-0.99985, -0.01056,0.005337, -0.031727699*1],
        #                            [0.0054654, -0.012091, 0.9998, 0.11451644*1],
        #                            [0.0, 0.0, 0.0, 1.0]])
        # self.tool2c_rt = np.array([[0, 1, 0, -0.0780857 * 1],
        #                            [-1, 0, 0, -0.031727699 * 1],
        #                            [0, 0, 1, 0.11451644 * 1],
        #                            [0.0, 0.0, 0.0, 1.0]])
    ...
    def run(self):
        ...
        with open(self.filename, 'rt', newline='', encoding='utf-8', errors='ignore') as f:
            ...
            for lines in f_csv:
                ...
                # depth_file = '2022-09-27-15-01-37/depth/' + str(i - 1).zfill(5) + '.png'
                # color_file = '2022-09-27-15-01-37/color/' + str(i - 1).zfill(5) + '.jpg'
                depth_file = 'ply/' + str(i - 1).zfill(5) + '.png'
                color_file = 'ply/' + str(i - 1).zfill(5) + '.jpg'
                ...
                if i == self.start_frame:
                    ...
                    bound1 = o3d.geometry.AxisAlignedBoundingBox([0, 0, 0], [1, 1, 1])
                    bound2 = o3d.geometry.AxisAlignedBoundingBox([-1, -1, 0], [1, 0, 1])
                    source = source.crop(bound1) + source.crop(bound2)
                    # source = o3d.geometry.PointCloud.create_from_depth_image(depth_image,intrinsic,project_valid_depth_only=2)
                    # o3d.visualization.draw_geometries([source])
                    # source.voxel_down_sample(self.voxel_size)
                    # s_x, s_y, s_z, s_rw, s_rx, s_ry, s_rz = lines
                    s_x, s_y, s_z, s_rx, s_ry, s_rz = lines
                    # S_R, S_T = self.Euler_to_Matrix(s_x, s_y, s_z, s_rx, s_ry, s_rz)
                    S_T = np.array([[float(s_x.strip("\ufeff")) / 1000], [float(s_y) / 1000], [float(s_z) / 1000]])
                    ...
                    bound1 = o3d.geometry.AxisAlignedBoundingBox([0, 0, 0], [1, 1, 1])
                    bound2 = o3d.geometry.AxisAlignedBoundingBox([-1, -1, 0], [1, 0, 1])
                    target = target.crop(bound1) + target.crop(bound2)
                    ...
                    t_x, t_y, t_z, t_rx, t_ry, t_rz = lines
                    T_T = np.array([[float(t_x.strip("\ufeff")) / 1000], [float(t_y) / 1000], [float(t_z) / 1000]])
                    ...
                    # T_T = np.array([[float(t_x) / 1000], [float(t_y) / 1000], [float(t_z) / 1000]])
